...
config = {
    ...
    "scheduler_params": {"T_max": 1000, "eta_min": 5.0e-4},
}
...
class SEDTrainer(object):
    ...
    def _train_step(self, batch):
        ...
        x = batch["X"].float().to(self.device)  # (B, mel, T')
        y_clip = batch["y"].float().to(self.device)
        ...
        self.train_pred_logit_epoch = np.concatenate(
            [
                self.train_pred_logit_epoch,
                y_["framewise_logit"].detach().cpu().numpy().max(1),
            ],
            axis=0,
        )
    ...
    def _train_epoch(self):
        ...
        try:
            self.epoch_train_loss["train/epoch_f1_02_frame"] = metrics.f1_score(
                ...
                sigmoid(self.train_pred_logit_epoch) > 0.2,
                average="samples",
                zero_division=0,
            )
            self.epoch_train_loss["train/epoch_f1_01_frame"] = metrics.f1_score(
                self.train_y_epoch > 0,
                sigmoid(self.train_pred_logit_epoch) > 0.1,
                average="samples",
                zero_division=0,
            )
    ...
    def _valid_epoch(self):
        ...
        try:
            ...
            self.epoch_valid_loss["valid/epoch_f1_01_frame"] = metrics.f1_score(
                self.valid_y_epoch > 0,
                sigmoid(self.valid_pred_logit_epoch) > 0.1,
                average="samples",
                zero_division=0,
            )
        except ValueError:
